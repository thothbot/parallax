...
public class ExtrudeGeometry extends Geometry
{
	...
	private static final float RAD_TO_DEGREES = (float)(180.0 / Math.PI);
	...
	{
		Parallax.app.debug("ExtrudeGeometry", "Called addShape() shape=" + shape);
		...
		{
			float t = b / (float)options.bevelSegments;
			float z = options.bevelThickness * ( 1.0f - t );

			float bs = options.bevelSize * (float)( Math.sin ( t * Math.PI / 2.0 ) ) ; // curved
...
//		bs = bevelSize;
		...
		for ( int b = options.bevelSegments - 1; b >= 0; b -- ) 
		{
			...
			float bs = options.bevelSize * (float)Math.sin ( t * Math.PI/2.0 ) ;
	...
	{
		float anglea = (float)Math.atan2( pt_j.getY() - pt_i.getY(), pt_j.getX() - pt_i.getX() );
		float angleb = (float)Math.atan2( pt_k.getY() - pt_i.getY(), pt_k.getX() - pt_i.getX() );

		if ( anglea > angleb )
			angleb += Math.PI * 2.0;

		float anglec = ( anglea + angleb ) / 2.0f;
	...
	private Vector2 scalePt2 ( Vector2 pt, Vector2 vec, float size ) 
	{
	...
	private Vector2 getBevelVec2( Vector2 pt_i, Vector2 pt_j, Vector2 pt_k ) 
	{
		...
		if ( v_dot_w_hat == 0 ) 
		{
			Parallax.app.debug("ExtrudeGeometry", "getBevelVec2() Either infinite or no solutions!");

			if ( q_sub_p_dot_w_hat == 0 )
				Parallax.app.debug("ExtrudeGeometry", "getBevelVec2() Its finite solutions.");

			else
				Parallax.app.error("ExtrudeGeometry", "getBevelVec2() Too bad, no solutions.");
		}
	...
	private void buildLidFaces() 
	{
		...
		Parallax.app.debug( "ExtrudeGeometry", "buildLidFaces() faces=" + flen);

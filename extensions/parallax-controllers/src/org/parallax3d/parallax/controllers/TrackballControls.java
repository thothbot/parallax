 ...
 *
 */
public final class TrackballControls extends Controls implements TouchMoveHandler, TouchDownHandler, TouchUpHandler, KeyDownHandler, KeyUpHandler
{
	...
	public TrackballControls(Object3D object, RenderingContext context)
	{
	...
	/**
	 * The method must be called in the {@link org.parallax3d.parallax.Animation#onUpdate(RenderingContext)}} onUpdate method.
	 */
	public void update() 
	{
	...
	public void onTouchDown(int screenX, int screenY, int pointer, int button)
	{
		...
		if ( state == STATE.NONE ) 
		{
			...
			{
				rotateStart = rotateEnd = getMouseProjectionOnBall( screenX, screenY );
				state = STATE.ROTATE;
			} 
			else if (button == Input.Buttons.MIDDLE && this.isZoom )
			{
				zoomStart = zoomEnd = getMouseOnScreen( screenX, screenY );
				state = STATE.ZOOM;
			} 
			else if ( button == Input.Buttons.RIGHT && this.isPan )
			{
				panStart = panEnd = getMouseOnScreen( screenX, screenY );
				state = STATE.PAN;
			}
		}
	}
	...
	public void onTouchMove(int screenX, int screenY, int pointer)
	{
		...
		{
			rotateEnd = getMouseProjectionOnBall( screenX, screenY );
		}
		else if ( this.state == STATE.ZOOM && this.isZoom ) 
		{
			zoomEnd = getMouseOnScreen( screenX, screenY );
		}
		else if ( this.state == STATE.PAN && this.isPan ) 
		{
			panEnd = getMouseOnScreen( screenX, screenY );
		}
	}

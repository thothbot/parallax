...
public class TransformControls extends Object3D implements TouchDownHandler, TouchMoveHandler, TouchUpHandler {
	...
	public void onTouchUp(int screenX, int screenY, int pointer, int button) {
		...
		_dragging = false;
		onPointerHover( screenX,  screenY);
	}
	...
	public void onPointerMove(int screenX, int screenY) {
		//onPointerMove
		...
		Raycaster.Intersect planeIntersect = intersectObjects(screenX, screenY, tmpList);
	...
	public void onTouchDown(int screenX, int screenY, int pointer, int button) {
		...
		if(button == Input.Buttons.LEFT) {
			Raycaster.Intersect intersect = this.intersectObjects(screenX, screenY, this.getTransformGizmo(_mode).pickers.getChildren());
			if(intersect!=null && intersect.object!=null) {
				...
				List<Object3D> tmpList=new ArrayList<Object3D>();
				tmpList.add(this.getTransformGizmo(_mode).activePlane);
				Raycaster.Intersect planeIntersect = this.intersectObjects(screenX, screenY,tmpList);
	...
	private Raycaster.Intersect intersectObjects(int screenX, int screenY, List<Object3D> objects ) {

		double posX = (screenX / (double) this.context.getWidth() ) * 2.0 - 1.0;
		double posY = - (screenY / (double) this.context.getHeight() ) * 2.0 + 1.0;
...
//			this.gizmoRotate.show();
	...
	abstract class TransformGizmo extends Object3D {
		...
		public void init() {
			...
			for(Map.Entry<String, Mesh> entry:planesLocal.entrySet()) {
	...
	public class GizmoLineMaterial extends org.parallax3d.parallax.graphics.materials.LineBasicMaterial implements HasHighlight {

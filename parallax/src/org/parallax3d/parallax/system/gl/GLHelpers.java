...
public class GLHelpers {
    ...
    public static int getProgramParameter(GL20 gl, int program, int type)
    {
        IntBuffer buffer = Int32Array.create(3).getTypedBuffer();
        gl.glGetProgramiv( program, type, buffer );
        return buffer.get(0);
    }

    public static boolean isProgramLinked(GL20 gl, int program)
    {
        return getProgramParameter(gl, program, GL20.GL_LINK_STATUS) > 0;
    }

    public static int getProgramValidStatus(GL20 gl, int program)
    {
        return getProgramParameter(gl, program, GL20.GL_VALIDATE_STATUS);
    }
}

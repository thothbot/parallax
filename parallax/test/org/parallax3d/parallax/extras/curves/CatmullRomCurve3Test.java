...
public class CatmullRomCurve3Test {
    ...
    static void vectorsAreEqual(List<Vector2> check, List<Vector3> that) {
        ...
        for (int i = 0; i < check.size(); i++) {
            ...
            assertTrue("Vector differs at index " + i + ". Should be " + a + " instead of " + b,
                    a.distanceToSquared(b) < threshold );
        }

    }

    private List<Vector3> positions = Arrays.asList(
    ...
    public void testCatmullrom_check() {
        CatmullRomCurve3 curve = new CatmullRomCurve3(positions)
                .setType(CatmullRomCurve3.Type.catmullrom);

        List<Vector3> catmullPoints = Arrays.asList(
                new Vector3(-60, -100, 60),
    ...
    public void testChordal_basic_check() {
        CatmullRomCurve3 curve = new CatmullRomCurve3(positions)
                .setType(CatmullRomCurve3.Type.chordal);

        List<Vector3> chordalPoints = Arrays.asList(
                new Vector3(-60, -100, 60),
    ...
    public void testCentripetal_basic_check() {
        CatmullRomCurve3 curve = new CatmullRomCurve3(positions)
                .setType(CatmullRomCurve3.Type.centripetal);

        List<Vector3> centripetalPoints = Arrays.asList(
                new Vector3(-60, -100, 60),

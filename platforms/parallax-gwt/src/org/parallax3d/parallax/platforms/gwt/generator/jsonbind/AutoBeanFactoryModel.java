...
public class AutoBeanFactoryModel {
  ...
  public AutoBeanFactoryModel(TreeLogger logger, JClassType factoryType)
    ...
    // Process annotations
    {
      AutoBeanFactory.Category categoryAnnotation = factoryType.getAnnotation(AutoBeanFactory.Category.class);
      ...
      AutoBeanFactory.NoWrap noWrapAnnotation = factoryType.getAnnotation(AutoBeanFactory.NoWrap.class);
      if (noWrapAnnotation != null) {
        processClassArrayAnnotation(noWrapAnnotation.value(), noWrapTypes);
      }

      EnumMap.ExtraEnums extraEnumsAnnotation = factoryType.getAnnotation(EnumMap.ExtraEnums.class);
    ...
    for (Class<?> clazz : classes) {
      ...
      if (category == null) {
        poison("Could not find @%s type %s in the TypeOracle",
            AutoBeanFactory.Category.class.getSimpleName(), clazz.getCanonicalName());
        continue;
